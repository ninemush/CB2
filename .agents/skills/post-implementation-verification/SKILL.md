...
|------|-------------|-------------|
| `manifest.json` | Idea metadata, generation run ID, timestamps, project name | Always |
| `<ProjectName>.nupkg` | The generated UiPath NuGet package | When build succeeded and cache available |
| `dhg.md` | The Developer Handoff Guide | When DHG was generated |
| `quality-gate-results.json` | Full quality gate output (violations, readiness, completeness) | When quality gate ran |
| `meta-validation-results.json` | Meta-validation output (corrections applied/skipped/failed, confidence score) | When meta-validation ran and cache available |
...
- **Cache dependency**: Some artifacts (nupkg buffer, meta-validation, final quality report) are only available from the in-memory pipeline cache. After a server restart, these artifacts will be absent from the bundle even if a generation run exists in the database.
