...
**Date:** 2026-03-16  
**Summary:** 12 distinct bugs found in AI-generated XAML that prevented the project from loading in UiPath Studio 25.x. All bugs required manual PowerShell patching to fix. Every bug is a systematic generator error — not a one-off — meaning it affects every workflow the generator produces.
...
1. **Bug #9** — ViewStateManager: easiest win, single change, affects every file
2. **Bug #4** — InvokeWorkflowFile prefix: affects every multi-file project
3. **Bug #5** — InvokeWorkflowFile arguments: affects every sub-workflow call
4. **Bug #1** — ContinueOnError: affects every generated activity
5. **Bug #3** — Message quote format: affects every LogMessage
6. **Bug #8** — Switch/Case: affects every Switch activity
7. **Bug #2** — While/RetryScope body: affects every loop/retry
8. **Bug #7** — AddLogFields type: affects every logging pattern
9. **Bug #11** — ExcelApplicationScope body: affects every Excel workflow
10. **Bug #6** — TakeScreenshot property: affects screenshot activities
11. **Bug #10** — DU package dependency: affects Document Understanding workflows
12. **Bug #12** — Package versions: requires version compatibility matrix
...
`FixSwitchCases.ps1`, `FixViewStateManager.ps1`
