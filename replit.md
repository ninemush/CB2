...
- **Activity Catalog System**: A versioned activity catalog (`catalog/activity-catalog.json`, v1.0.0) serves as the single source of truth for activities, properties, packages, and enum constraints. Each property includes `validValues` for enum constraints and optional `addedInVersion`/`removedInVersion` for version-specific availability. Each package includes `feedStatus` ("verified"|"unverified") and `preferredVersion`. The catalog service (`server/catalog/catalog-service.ts`) exposes query methods (`getEnumValues`, `isPropertyAvailableForVersion`, `getPreferredVersion`, `getFeedStatus`) and performs a 90-day freshness check on `lastVerifiedAt`. The `ACTIVITY_REGISTRY` in `server/uipath-activity-registry.ts` delegates to the catalog via Proxy when loaded (with generation-counter-based cache invalidation), falling back to a hardcoded registry. The hardcoded `UNVERIFIED_PACKAGES` set is eliminated — feed status now lives in the catalog. The template builder supports a wide mode (all activities regardless of processType) and a compact prompt format for token-budget-constrained scenarios.
