...
- **V2 UiPath Integration Layer**: Hardened auth module (`server/uipath-auth.ts`) with **dual-token architecture** — UiPath's identity server requires separate tokens per resource (OR.* and TM.* scopes cannot be combined in one token request). OR token (cached via `getToken()`/`getHeaders()`) for Orchestrator API calls, TM token (cached via `getTmToken()`/`getTmHeaders()`) for Test Manager API calls. Both have singleton caching, proactive 60s-before-expiry refresh, 401 auto-retry with token-type-aware invalidation, and secret masking. `makeAuthenticatedRequest()` auto-selects the correct token based on URL (`testmanager_` → TM token, otherwise → OR token). Service-specific base URL helpers: `getBaseUrl` (Orchestrator `orchestrator_`), `getTestManagerBaseUrl` (Test Manager `testmanager_`). Action Center uses the Orchestrator OData API (`{orchBase}/odata/TaskCatalogs` and `{orchBase}/odata/Tasks`) with OR.Tasks scopes, not a separate microservice. Test Manager calls use TM token for `testmanager_/api/v2/` endpoints. Users/Robot Accounts falls back to `/odata/Robots` when `GetUsersForFolder` is unavailable (404 on modern tenants). Typed Orchestrator API client (`server/orchestrator-client.ts`) with exponential backoff retry (3x), 30s timeout, and full method coverage. Prerequisite checker (`server/prerequisite-checker.ts`) with 6 checks returning blocking/warning/pass statuses. New API endpoints: `/api/uipath/health`, `/api/uipath/live-ops`, `/api/uipath/diagnostics`, `/api/uipath/prerequisites`, provisioning status/emergency-stop, action center tasks, test results.
