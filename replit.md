...
Key files: `server/uipath-auth.ts` (token management), `server/uipath-deploy.ts` (provisioning), `server/uipath-integration.ts` (service probing), `server/uipath-routes.ts` (API routes), `server/document-routes.ts` (PDD/SDD generation with platform capabilities).