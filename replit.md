...
-   **LLM-Based Intent Classification**: Uses a lightweight LLM call to classify user intent (PDD, SDD, DEPLOY, CHAT) to drive appropriate system actions and document generation. A pre-classification keyword check short-circuits obvious requests (e.g. "generate the SDD") without needing the LLM, preventing misclassification from conversation context.
-   **Document Approval**: Supports approval of PDD/SDD via dedicated buttons, chat phrases, or auto-chain detection. All three paths converge on a single canonical flow: approval is recorded, then the auto-chain `useEffect` detects it (checking both assistant and system messages) and triggers the next generation step with proper progress indicator. Chat-phrase approval uses two-function detection: `hasApprovalIntent()` (boolean, excludes map-related phrases like "to-be"/"as-is"/"map") and `getExplicitDocType()` ("PDD"/"SDD"/null). When intent is detected, `resolveApprovalTarget()` determines which document to approve based on current pipeline state (draft SDD first, then draft PDD). If a document was displayed in chat but not persisted to the DB, a fallback recovers it from chat history before approving.
...
-   **Process Map Auto-Clone Guard**: To-be auto-clone from as-is uses a DB-first check: always queries to-be nodes before cloning. If to-be already has nodes, clone is suppressed — no dependency on in-memory state. Bulk endpoint acquires `toBeGenerationLocks` BEFORE clearing, preventing GET auto-clone race condition. Write-side mutual exclusion via `bulkWriteLocks` serializes concurrent bulk requests for the same idea+view.
...
-   **Process Map Dead-End Auto-Repair**: Three-layer fix for dead-end branches (nodes reachable from start but not terminating at an end node). (1) AI prompt explicitly forbids dead-end branches in BRANCHING RULES and SELF-CHECK. (2) Client-side: after building edges from parsed steps, detects leaf nodes (no outgoing edges, not end/start) and auto-wires them to the first end node before sending to bulk API. (3) Server-side: `repairDeadEndBranches()` runs after `cleanupUnreachableNodes` in bulk creation and at startup, catching any dead-ends that survive client-side processing.
