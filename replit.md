...
- **V2 UiPath Integration Layer**: Hardened auth module (`server/uipath-auth.ts`) with singleton token caching, proactive 60s-before-expiry refresh, 401 auto-retry, secret masking, and expanded default scopes covering Orchestrator (`OR.*`), Action Center (`AC.*`), and Test Manager (`TM.*`) in a single cached token. Service-specific base URL helpers: `getBaseUrl` (Orchestrator `orchestrator_`), `getTestManagerBaseUrl` (Test Manager `testmanager_`). Action Center uses the Orchestrator OData API (`{orchBase}/odata/TaskCatalogs` and `{orchBase}/odata/Tasks`), not a separate microservice. Test Manager probes use Orchestrator OData (`/odata/TestSets`) as primary (works with OR.TestSets scopes), with `testmanager_/api/v2/` as optional upgrade when TM scopes are granted. Users/Robot Accounts falls back to `/odata/Robots` when `GetUsersForFolder` is unavailable (404 on modern tenants). Typed Orchestrator API client (`server/orchestrator-client.ts`) with exponential backoff retry (3x), 30s timeout, and full method coverage (queues, processes, jobs, robots, action center via Orchestrator OData, test manager via Orchestrator OData + optional `testmanager_/api`, triggers, monitoring). Prerequisite checker (`server/prerequisite-checker.ts`) with 6 checks (machines, robots, folder, package feed, action center, test manager) returning blocking/warning/pass statuses. New API endpoints: `/api/uipath/health`, `/api/uipath/live-ops`, `/api/uipath/diagnostics`, `/api/uipath/prerequisites`, provisioning status/emergency-stop, action center tasks, test results.
