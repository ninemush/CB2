...
- **Generation Modes**: Supports `baseline_openable` (default: skips AI enrichment, flat scaffold) and `full_implementation` (AI enrichment, REFramework). Pipeline defaults to `baseline_openable` and upgrades to `full_implementation` only when a pre-check confirms all activities are emission-approved and complexity is simple-tier. Existing auto-downgrade safety net (`full_implementation → baseline_openable` on quality gate failure, excessive stripping, or archive errors) is preserved. Pipeline instrumentation logs mode selection, upgrade attempts, and downgrade events.
- **SDD Business Context Mapper**: (`server/sdd-business-context-mapper.ts`) Extracts per-workflow business context from SDD content, keyed by workflow file name. Prioritizes structured signals (workflow names → SDD section headings, step/activity names, integration references, retry/error handling markers, I/O declarations, config/asset references) with prose description as secondary fallback.
- **Business-Context-Aware Handoff Stubs**: The emission gate receives per-workflow business context and annotates handoff stubs (both activity-level and block-level) with business descriptions: what the block was supposed to do, integration references, error handling patterns, and I/O declarations.
- **Business-Meaning-Aware Degradation**: Degradation happens at the smallest granularity that preserves business meaning: activity-level stubs when surrounding flow is unaffected, block-level replacement for indivisible business containers (RetryScope, TryCatch), workflow-level flagging only when heavily degraded (>60% activities stubbed).
...
- **Canonical XAML Normalization Boundary**: All bracket-wrapping (`ensureBracketWrapped`, `smartBracketWrap`), literal-vs-expression classification (`looksLikePlainText`, `looksLikeVariableRef`), bare-word handling (`BARE_WORD_LITERALS_SET`), and package/prefix/namespace mapping (`CLR_NAMESPACE_TO_XAML_PREFIX`, `PACKAGE_NAMESPACE_MAP`) are defined once in `server/xaml/xaml-compliance.ts`. All other files import from this single canonical source. `CLR_NAMESPACE_TO_XAML_PREFIX` is derived from `PACKAGE_NAMESPACE_MAP` at module init, ensuring they cannot diverge. A pre-emit normalization invariant check (`checkNormalizationInvariants` in `server/emission-gate.ts`) blocks emission if typed property objects leak into XAML, raw CLR types appear where XAML-prefix types are required, VB expression defaults are unwrapped, generic type serialization is malformed, or mixed literal/expression attribute syntax is detected.
- **Unified Declaration Registry**: A single `DeclarationRegistry` class (`server/declaration-registry.ts`) is the source of truth for all variable and argument declarations during workflow assembly. It consolidates 5 declaration sources (explicit spec, implicit output, discovered reference, iterator, catch-exception) with scope tracking (workflow vs block), type inference, and conflict detection. A pre-emission expression walk discovers both argument references and prefixed variable references from spec nodes before XAML serialization, with a post-emission safety net for template-generated references.
- **Canonical Type Inference**: A single shared `inferTypeFromPrefix()` function (`server/shared/type-inference.ts`) is the source of truth for mapping variable naming convention prefixes (str_, int_, dt_, qi_, arr_, list_, etc.) to UiPath XAML types. All three previous copies (workflow-tree-assembler, xaml-generator, deterministic-validators) delegate to this canonical function. The `qi_` prefix maps to `ui:UiPath.Core.QueueItem`, `arr_` to `x:Array(x:String)`, and `list_` to `scg:List(x:String)`.
- **DHG Tier Classification**: The Developer Handoff Guide distinguishes four workflow tiers: Generated (fully generated), Handoff (has handoff blocks), Blocked (real content but not Studio-openable), and Stub (actual stub content). The parity checker validates all four tiers against archive content.
- **Undeclared Variable Auto-Repair**: Variables with recognized naming prefixes are auto-declared during pre-emission tree assembly, in the correction-applier fallback, and in the XAML compliance pass. Unprefixed undeclared variables are routed to the DHG as `UNDECLARED_VARIABLE_MANUAL` remediations instead of causing package stubs. Workflows whose only blocking violations are undeclared variables remain structurally Studio-openable.
...
- **Authoritative Namespace Injection**: A single authoritative pass runs after all XAML files are generated, scanning every file for activity tag prefixes, looking up required xmlns/assembly/namespace in the catalog, and injecting any missing declarations. Replaces fragmented injection across multiple stages. All three injection sites (`runAuthoritativeNamespaceInjection` in `package-assembler.ts`, `Authoritative Declaration Synthesis` in `package-assembler.ts`, and `injectDynamicNamespaceDeclarations` in `xaml-compliance.ts`) have self-healing: when `insertBeforeClosingCollectionTag` fails because a declaration block exists but has a corrupted/missing `</sco:Collection>` tag, the block is rebuilt from scratch by extracting existing entries via regex and merging with new entries.
